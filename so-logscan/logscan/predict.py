...
def produce_alerts(model, dataset, split_data):
    ...
    for i in range(len(dataset)):
        ...
        if y >= 0.5:
            ...
            current_time = dt.datetime.now().strftime("%m-%d-%Y %-l:%M%p ") + tz_string
            ...
            alert_data.append([start_timestamp, source_ip, start_time, end_time, num_attempts, num_failed, current_time])
...
def write_json_alerts(alert_data, outfile_path):
    lines = [json.dumps({'Timestamp': i[6], 'Source IP': i[1], 'Start Time': i[2], 'End Time': i[3], 'Number of Login Attempts': i[4], 'Number of Failed Login Attempts': i[5]}) for i in alert_data]
