...
from elastalert.util import (add_raw_postfix, cronite_datetime_to_timestamp, dt_to_ts, dt_to_unix, EAException,
                             elastalert_logger, elasticsearch_client, eql_client, format_index, lookup_es_key, parse_deadline,
...
class ElastAlerter(object):
    ...
    def __init__(self, args):
        ...
        self.eql = self.conf.get('eql', False)
        ElastAlerter.eql = self.eql
    ...
    @staticmethod
    def get_query(filters, starttime=None, endtime=None, sort=None, timestamp_field='@timestamp', to_ts_func=dt_to_ts, desc=False):
        ...
        if ElastAlerter.eql:
            if starttime and endtime:
                query = {'filter': { 'range': {timestamp_field: {'gt': starttime, 'lte': endtime}}}, 'query': new_filters[0]['query_string']['query']}
            else:
                query = {'query': new_filters[0]['query_string']['query']}
        else:                                                     
            if starttime and endtime:
        ...
        return query
    def get_terms_query(self, query, rule, field, size=None):
    ...
    def get_hits(self, rule, starttime, endtime, index, scroll=None):
        ...
        try: 
            if scroll:
                ...
                if self.eql:
                    res = self.thread_data.current_eql.search(
                        index=index,
            ...
            if len(res.get('_shards', {}).get('failures', [])) > 0:
                try:
                    ...
                    raise ApiError(str(res['_shards']['failures']))
    ...
    def get_hits_terms(self, rule, starttime, endtime, index, key, qk=None, size=None):
        ...
        if self.eql:
            ...
            query = self.get_terms_query(base_query, rule, key)

            try:
                res = self.thread_data.current_eql.search(index=index, body=query, ignore_unavailable=True)
            except (ApiError, TransportError) as e:
                # Elasticsearch sometimes gives us GIGANTIC error messages
                # (so big that they will fill the entire terminal buffer)
                if len(str(e)) > 1024:
            ...
            if size is None:
                size = rule.get('terms_size', 50)
            query = self.get_terms_query(base_query, rule, key, size)
    ...
    def get_hits_aggregation(self, rule, starttime, endtime, index, query_key, term_size=None):
        ...
        try:
            if self.eql:
                res = self.thread_data.current_es.search(index=index, body=query, ignore_unavailable=True)
            else:
                es = self.thread_data.current_es.search(index=index, body=query, size=0, ignore_unavailable=True)
        except (ApiError, TransportError) as e:
            if len(str(e)) > 1024:
                e = str(e)[:1024] + '... (%d characters removed)' % (len(str(e)) - 1024)
    ...
    def get_eql_client(self, rule):
        key = rule['name']
        eqlClient = self.eql_clients.get(key)
        if eqlClient is None:
            eqlClient = eql_client(rule)
            self.eql_clients[key] = eqlClient
        return eqlClient
    ...
    def run_rule(self, rule, endtime, starttime=None):
        ...
        self.thread_data.current_eql = self.get_eql_client(rule)
    ...
    def send_pending_alerts(self):
        ...
        for alert in pending_alerts:
            ...
            if ts_now() > ts_to_dt(alert_time):
                ...
                except (ApiError, TransportError):  # TODO: Give this a more relevant exception, try:except: is evil.
    ...
    def get_aggregated_matches(self, _id):
        ...
        except (KeyError, (ApiError, TransportError)) as e:
    ...
    def is_silenced(self, rule_name):
        ...
        try:
            ...
            index = self.writeback_es.resolve_writeback_index(self.writeback_index, doc_type)
            if self.eql:
                res = self.writeback_es.search(index=index, body=query,
