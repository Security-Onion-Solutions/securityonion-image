...
from elastalert import EqlClient
...
def elasticsearch_client(conf):
    ...
    if es_conn_conf['es_bearer'] or es_conn_conf['es_api_key']:
        username = None
        password = None
    es_conn_conf['http_auth'] = Elasticsearch(es_conn_conf['es_hosts'],
...
def eql_client(conf):
    ...
    return EqlClient(es_conn_conf)

def build_es_conn_config(conf):
    """ Given a conf dictionary w/ raw config properties 'use_ssl', 'es_hosts', 'es_port'
    'es_username' and 'es_password', this will return a new dictionary
    with properly initialized values for 'es_hosts', 'es_port', 'use_ssl' and 'http_auth' which
    ...
    es_hosts = parse_hosts(es_hosts) if es_hosts else conf.get('es_hosts')
